...
internal class NotepadPlusPlus : ExternalSourceEditorBase
{
    ...
    public override bool IsAvailable
        => !string.IsNullOrEmpty(GetEditorExecutable(ExecutablePath));

    public override bool OpenFile(string file, int lineNumber, int initiatorProcessId)
    {
        var executable = GetEditorExecutable(ExecutablePath);
        if (executable == null) return false;

        var argsBuilder = new StringBuilder();

        if (lineNumber > 0) argsBuilder.Append("-n").Append(lineNumber);
