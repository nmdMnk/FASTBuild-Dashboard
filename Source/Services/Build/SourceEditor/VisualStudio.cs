...
internal partial class VisualStudio : IExternalSourceEditor
{
    ...
    {
        var dteInstances = RetrieveDteInstances();

        // prior use if a DTE instance with matching process ID found
        if (initiatorProcessId > 0)
            if (TryOpenWithDte(
                    dteInstances,
                    d => MatchesInitiatorProcessId(d, initiatorProcessId),
                    file,
                    lineNumber,
                    true))
                return true;
        ...
        SortByPseudoSolutionMatching(dteInstances, file);
        if (TryOpenWithDte(dteInstances, null, file, lineNumber)) return true;
        ...
        foreach (var progId in DteProgIds)
        {
            ...
            try
            {
                ...
                if (TryOpenWithDte(dte, file, lineNumber)) return true;
            }
            catch (COMException)
            {
                // skip
    ...
    private static List<DteInstanceInfo> RetrieveDteInstances()
    {
        ...
        var retVal = WinAPI.GetRunningObjectTable(0, out var rot);

        if (retVal != 0) return candidates;
        ...
        while (enumMoniker.Next(1, monikers, IntPtr.Zero) == 0)
        {
            ...
            if (!DteProgIds.Any(progId => displayName.StartsWith($"!{progId}:"))) continue;
        ...
        bool distinct = false)
    {
        ...
        {
            if (condition != null && !condition(instanceInfo)) continue;

            if (TryOpenWithDte(instanceInfo.DteInstance, file, lineNumber)) return true;
