...
internal class BuildSessionJobManager
{
    ...
    private readonly Dictionary<string, BuildJobViewModel> _activeRemoteJobMap = new();
    ...
    {
        if (!job.OwnerCore.OwnerWorker.IsLocal) return null;
    ...
    {
        foreach (var job in GetAllJobs().Where(j => !j.IsFinished)) NotifyJobFinished(job);
    }

    private class JobStartTimeComparer : IComparer<BuildJobViewModel>
    {
        public static readonly JobStartTimeComparer Instance = new();
